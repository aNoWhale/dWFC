...
@partial(jax.jit, static_argnames=['n_cells', 'tau','sigma', 'compat_threshold'])  # 新增兼容度相关静态参数
def soft_mask(index, n_cells, tau=2.0, sigma=1.0, compat_threshold=1e-3, alpha_compat=5.0):  # 新增兼容度参数（带默认值）
...
@partial(jax.jit, static_argnames=['tau', 'compat_threshold'])  # 新增兼容度静态参数
def single_update_by_neighbors(
    collapse_idx, key, log_init_probs, A, D, dirs_opposite_index, 
    log_compatibility, alpha=0., tau=2.0, compat_threshold=1e-3, alpha_compat=5.0  # 新增兼容度参数（带默认值）
):
    ...
    total_neighbors = jnp.sum(neighbor_mask) + 1e-8  # 总有效邻居数（加epsilon避免除0）
    ...
    # -------------------------- 核心新增：兼容度权重计算 --------------------------
    # 步骤1：将log空间的贡献转换为普通空间，判断是否为兼容贡献
    sum_factors = jnp.exp(log_sum_factors)  # (n_cells, n_tiles)
    # 兼容贡献掩码：>阈值为1（兼容），<=阈值为0（不兼容）
    compat_contrib_mask = (sum_factors > compat_threshold).astype(jnp.float32)  # (n_cells, n_tiles)
    # 步骤2：计算每个tile的兼容邻居数（有效邻居中贡献兼容的数量）
    compat_neighbor_count = jnp.sum(compat_contrib_mask * neighbor_mask[:, None], axis=0)  # (n_tiles,)
    # 步骤3：计算兼容度得分（兼容邻居数/总邻居数）
    compat_score = compat_neighbor_count / total_neighbors  # (n_tiles,)
    # 步骤4：兼容度权重（log空间，乘法变加法；alpha_compat控制权重强度）
    log_compat_weight = alpha_compat * jnp.log(compat_score + 1e-8)  # (n_tiles,)，加epsilon避免log(0)
    # -----------------------------------------------------------------------------
    
    # 温度系数适配：普通空间(x)^tau → log空间tau * log(x)
    log_tau_sum_factors = tau * log_sum_factors  # (n_cells, n_tiles)
    # 叠加兼容度权重（关键：让兼容度高的tile总贡献更高）
    log_tau_sum_factors = log_tau_sum_factors + log_compat_weight[None, :]  # 广播到(n_cells, n_tiles)
...
@jax.jit
def waveFunctionCollapse(
    init_probs, A, D, dirs_opposite_index, compatibility, key, 
    tau=2.0, compat_threshold=1e-3, alpha_compat=5.0  # 新增兼容度参数（带默认值）
):  # 保持原有签名，仅新增兼容度参数（带默认值）
    """WFC主函数：用vmap批量处理，适配可变邻居数 + 兼容度权重"""
    ...
    # 3. 第一步：批量更新所有单元（用vmap替代scan，传递兼容度参数）
    subkeys = jax.random.split(key, n_cells)  # 每个单元独立密钥
    # 用vmap并行处理所有单元的update_by_neighbors（传递兼容度参数）
    batch_updated_step1 = jax.vmap(
        single_update_by_neighbors,
        in_axes=(0, 0, None, None, None, None, None, None, None, None, None)  # 新增兼容度参数的in_axis=None
    )(
        ...
        0.,  # alpha参数
        tau, # 温度系数
        compat_threshold,  # 兼容贡献阈值
        alpha_compat       # 兼容度权重系数
    )
    # 聚合第一步结果（因迭代独立，取平均叠加）
    ...
    batch_updated_step2 = jax.vmap(
        ...
        in_axes=(0, None, None, None, None, None)  # tau的in_axis=None
    )(
        jnp.arange(n_cells),  # 所有单元索引
