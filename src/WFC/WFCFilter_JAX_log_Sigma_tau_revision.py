...
def spatial_soft_mask(target_cell_idx, cell_centers, sigma=0.1, neighbor_radius=1.0):
    ...
    :param target_cell_idx: 目标单元索引（标量）
    :param cell_centers: 所有单元的中心坐标 (n_cells, 3)
    :param sigma: 掩码衰减系数（越小，掩码越“陡峭”，越接近硬掩码）
    :param neighbor_radius: 邻域半径（定义“局部”范围，如1.0倍单元边长）
    :return: mask: (n_cells,) —— 连续值软掩码（非0/1，梯度可平滑传递）
    """
    # 1. 提取目标单元的中心坐标（与排序无关）
    target_center = cell_centers[target_cell_idx]  # (3,)
    
    # 2. 计算所有单元到目标单元的3D欧氏距离（真实空间距离，无排序依赖）
    dists = jnp.linalg.norm(cell_centers - target_center[None, :], axis=1)  # (n_cells,)
    
    # 3. 生成软掩码：仅在邻域半径内有非零值，且连续衰减（避免硬截断）
    # 第一步：过滤超出邻域半径的单元（软过滤，非硬0）
    in_radius = jax.nn.sigmoid(-(dists - neighbor_radius) / sigma)  # (n_cells,) —— 半径内≈1，外≈0
    # 第二步：距离衰减（邻域内越近，权重越高）
    distance_decay = jax.nn.sigmoid(-dists**2 / (2 * sigma**2))  # (n_cells,)
    # 合并：邻域过滤 + 距离衰减
    mask = in_radius * distance_decay  # (n_cells,) —— 连续值，无硬0/1
    
    # 4. 归一化（保证掩码和为1，适配WFC概率聚合）
    mask_sum = jnp.sum(mask)
    mask = jnp.where(mask_sum == 0, mask, mask / mask_sum)  # 避免除0
...
def compute_cell_centers(cell_vertices):
    ...
    :param cell_vertices: 形状 (n_cells, n_vertices_per_cell, 3) —— 每个单元的所有顶点坐标
    :return: cell_centers: 形状 (n_cells, 3) —— 每个单元的中心坐标（x,y,z）
    """
    # 对每个单元的所有顶点坐标求平均 → 中心坐标
    cell_centers = jnp.mean(cell_vertices, axis=1)  # (n_cells, 3)
    # 可选：坐标归一化（避免数值过大，提升稳定性）
    cell_centers = cell_centers / jnp.max(jnp.abs(cell_centers))  # 归一化到[-1,1]
    return cell_centers
...
def single_update_by_neighbors(collapse_idx, key, log_init_probs, cell_centers, A, D, dirs_opposite_index, log_compatibility, alpha=0., tau=2.0):
    ...
    # 1. 生成无排序依赖的空间软掩码（核心替换）
    collapse_mask = spatial_soft_mask(
        target_cell_idx=collapse_idx,
        cell_centers=cell_centers,
        sigma=0.1,  # 陡峭但连续，兼顾局部性和梯度
        neighbor_radius=1.0  # 3D单元边长，可根据实际调整
    )[:, None]  # (n_cells, 1)
...
def waveFunctionCollapse(init_probs, A, D, dirs_opposite_index, compatibility, key, cell_centers, tau=1.0,*args, **kwargs):  # 添加tau参数
    ...
    batch_updated_step1 = jax.vmap(
        ...
        in_axes=(0, 0, None, None,  None, None, None, None, None, None)  # 新增tau的in_axis=None
    )(
