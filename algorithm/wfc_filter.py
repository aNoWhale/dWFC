...
def update_by_neighbors(log_probs, collapse_idx, A, D, dirs_opposite_index, log_compatibility, log_init_probs, key, alpha=0.,): #0.3 
...
def waveFunctionCollapse(init_probs, A, D, dirs_opposite_index, compatibility):
    ...
    return final_probs, giveout1,giveout2
...
if __name__ == "__main__":
    ...
    init_probs = jnp.ones((num_elements, numTypes)) * 0.5
    key = jax.random.PRNGKey(0)
    noise = jax.random.uniform(key, init_probs.shape)
    init_probs = init_probs + (noise-0.5)*0.1
    from algorithm.wfc_filter import preprocess_adjacency,waveFunctionCollapse


    # 预构建邻接矩阵和方向矩阵（仅一次）
    A, D = preprocess_adjacency(adj, tileHandler)
    # 运行WFC
    probs, log_probs1, log_probs2 = waveFunctionCollapse(
        init_probs, A,D, tileHandler.opposite_dir_array,tileHandler.compatibility)
    probs1 = np.exp(log_probs1)
    probs2 = np.exp(log_probs2)
    np.savetxt('algorithm/init.csv',init_probs.T,delimiter=",")
    np.savetxt('algorithm/probs1.csv',probs1.T,delimiter=",")
    np.savetxt('algorithm/probs2.csv',probs2.T,delimiter=",")
